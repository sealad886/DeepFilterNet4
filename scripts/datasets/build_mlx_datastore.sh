...
Core options:
  ...
  --music-list PATH           Optional dedicated background music file list
                              (default: LIST_DIR/background_music_expanded.txt
                              when generated by download_datasets.sh,
                              otherwise LIST_DIR/background_music.txt)
...
if [[ -s "${LIST_DIR}/background_music_expanded.txt" ]]; then
...
if [[ -f "${MUSIC_LIST}" ]]; then
  ...
  if [[ "${MUSIC_LIST}" == "${LIST_DIR}/background_music_expanded.txt" ]]; then
    echo "Music flavor:       expanded (MUSAN + targeted FSD50K room/live/speaker clips)"
  fi
else
