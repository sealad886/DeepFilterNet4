...
from df.multiframe import MultiResolutionDF, AdaptiveOrderPredictor
...
class ModelParams4(DfParams):
    ...
    def __init__(self):
        ...
        self.mamba_d_conv: int = config(
            "MAMBA_D_CONV", cast=int, default=4, section=self.section
        )
        self.mamba_expand: int = config(
            "MAMBA_EXPAND", cast=int, default=2, section=self.section
        )
        ...
        self.max_df_order: int = config(
            "MAX_DF_ORDER", cast=int, default=7, section=self.section
        )
        self.min_df_order: int = config(
            "MIN_DF_ORDER", cast=int, default=2, section=self.section
        )
...
class ErbDecoder4(nn.Module):
    ...
    def __init__(
        ...
        self.conv0_out = conv_layer(
            conv_ch, 1, kernel_size=(1, 3), activation_layer=nn.Sigmoid
        )
    ...
    def forward(
        ...
        if self.run_df:
            if self.use_multi_res_df:
                ...
                df_coefs, order_weights, predicted_order = self.df_dec(
                    emb_active, c0_active
                )
                if self.lsnr_dropout and self.training and idcs is not None:
