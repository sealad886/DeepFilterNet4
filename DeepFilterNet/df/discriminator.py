...
class PeriodDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        for i, out_ch in enumerate(channels):
            ...
            self.convs.append(
                norm_f(nn.Conv2d(in_ch, out_ch, (5, 1), stride, padding=(2, 0)))
            )
            in_ch = out_ch
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        self.discriminators = nn.ModuleList([
            ScaleDiscriminator(use_spectral_norm=use_spectral_norm)
...
class CombinedDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        self.mpd = MultiPeriodDiscriminator(
            periods=periods, use_spectral_norm=use_spectral_norm
        )
