...
from df.evaluation_utils import evaluation_loop, get_metrics, enhance
...
def main():
    ...
    model4, df_state = load_model(
        args.checkpoint or "", 
        model_type="deepfilternet4"
    )
    ...
    if args.test_dir and not args.rtf_only:
        ...
        if args.compare_dfnet3:
            ...
            model3, df_state3 = load_model(
                args.compare_dfnet3,
                model_type="deepfilternet3"
            )
            
            params3 = count_parameters(model3)
            rtf3 = measure_rtf(model3, df_state3, duration_sec=args.rtf_duration)
    ...
    logger.info("="*60)
    logger.info(f" Metric       | Target | Achieved")
    logger.info(f" -------------|--------|--------")
