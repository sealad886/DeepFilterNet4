    ...
    *,
    ...
    if verbose:
        train_pbar.set_postfix(
            ...
            ctr=(f"{contrastive_loss_val:.4f}" if use_contrastive_awesome_loss else "0.0000"),
            mask=(
                f"{mask_mean:.2f}"
                if (use_awesome_loss or use_pipeline_awesome_loss or use_contrastive_awesome_loss)
                else "0.00"
            ),
            pos=(f"{contrastive_pos_sim_val:.3f}" if use_contrastive_awesome_loss else "0.000"),
            neg=(f"{contrastive_neg_sim_val:.3f}" if use_contrastive_awesome_loss else "0.000"),
            lr=f"{lr:.1e}",
