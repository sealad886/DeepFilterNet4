...
def test_gan_waveform_view_helper_exists() -> None:
    assert "def _gan_waveform_view(" in TRAIN_SOURCE
...
def test_discriminator_update_follows_optimizer_step_cadence() -> None:
    assert "do_disc_update = did_optimizer_update and ((global_step % gan_disc_update_freq) == 0)" in TRAIN_SOURCE


def test_generator_fm_real_branch_is_stop_grad() -> None:
    assert "disc_real, real_feats = discriminator(mx.stop_gradient(gan_clean_wav))" in TRAIN_SOURCE


def test_gan_paths_use_precision_view_before_discriminator() -> None:
    assert "pred_wav = _gan_waveform_view(pred_wav, use_fp16=bool(use_fp16))" in TRAIN_SOURCE
    assert "clean_wav = _gan_waveform_view(clean_wav, use_fp16=bool(use_fp16))" in TRAIN_SOURCE
