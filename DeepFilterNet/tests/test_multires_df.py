...
from df.multiframe import (
    DF,
    DFreal,
    MultiResolutionDF,
    AdaptiveOrderPredictor,
)
...
class TestMultiResolutionDF:
    ...
    def test_forward_shape(self, spec_tensor, default_resolutions):
        ...
        for num_freqs, frame_size in default_resolutions:
            ...
            coefs = torch.randn(
                spec_tensor.shape[0], frame_size, spec_tensor.shape[2], 
                num_freqs, 2
            )
    ...
    def test_gradient_flow(self, spec_tensor, default_resolutions):
        ...
        for num_freqs, frame_size in default_resolutions:
            ...
            coefs = torch.randn(
                spec_tensor.shape[0], frame_size, spec_tensor.shape[2],
                num_freqs, 2, requires_grad=True
            )
            coefs_list.append(coefs)
...
class TestAdaptiveOrderPredictor:
    ...
    def test_init_custom_orders(self):
        ...
        predictor = AdaptiveOrderPredictor(
            emb_dim=128, max_order=10, min_order=3
        )
    ...
    def test_eval_produces_one_hot(self, predictor_embedding):
        ...
        assert torch.allclose(
            order_weights.sum(dim=-1),
            torch.ones_like(order_weights.sum(dim=-1))
        )
    ...
    def test_predicted_order_range(self, predictor_embedding):
        ...
        predictor = AdaptiveOrderPredictor(
            emb_dim=256, max_order=7, min_order=2
        )
...
class TestAdaptiveDfDecoder:
    ...
    def test_forward_output_shapes(self, embedding_tensor, c0_tensor):
        ...
        assert coefs.shape == (
            c0_tensor.shape[0], 7, c0_tensor.shape[2], decoder.df_bins, 2
        )
        
        # order_weights: [B, T, num_orders]
        assert order_weights.shape == (
            c0_tensor.shape[0], c0_tensor.shape[2], decoder.num_orders
        )
        
        # predicted_order: [B, T]
        assert predicted_order.shape == (
            c0_tensor.shape[0], c0_tensor.shape[2]
        )
...
class TestDevices:
    ...
    def test_multires_df_cuda(self, spec_tensor, default_resolutions):
        ...
        for num_freqs, frame_size in default_resolutions:
            # [B, O, T, F, 2] format
            coefs = torch.randn(
                spec.shape[0], frame_size, spec.shape[2],
                num_freqs, 2, device="cuda"
            )
            coefs_list.append(coefs)
